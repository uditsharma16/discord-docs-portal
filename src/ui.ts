...
@media(max-width:650px){.identity{display:none}.toolbar{align-items:stretch;flex-direction:column}.login-card{padding:30px;margin-top:5vh}.document{border-radius:12px}.nav{height:64px}}
`;
...
export function documentPage(user: SessionUser, file: DriveFile, body: string): string {
  return layout(file.name, `<nav class="breadcrumb"><a href="/">Archive</a><span>›</span><span>${escapeHtml(file.name)}</span></nav><article class="document">${body}</article>`, user);
}
